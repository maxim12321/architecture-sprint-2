...
until docker exec -t -i router-1 sh -c 'mongosh --host shard1/shard1-1:27017 --eval "rs.status().members.some(m => m.stateStr === \"PRIMARY\")" | grep -q "true"; exit $?'; do
  sleep 5
done
...
until docker exec -t -i router-1 sh -c 'mongosh --host shard2/shard2-1:27017 --eval "rs.status().members.some(m => m.stateStr === \"PRIMARY\")" | grep -q "true"; exit $?'; do
  sleep 5
done
